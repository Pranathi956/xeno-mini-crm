...
router.get("/customers", async (req, res): Promise<void> => {
  ...
  if (search) {
    conditions.push(
      sql`(${ilike(customersTable.name, `%${search}%`)} OR ${ilike(customersTable.email, `%${search}%`)})`
    );
  }
  if (city) {
    conditions.push(ilike(customersTable.city, `%${city}%`));
  }
  if (minSpent != null) {
    conditions.push(gte(customersTable.totalSpent, String(minSpent)));
  }
  if (maxSpent != null) {
    conditions.push(lte(customersTable.totalSpent, String(maxSpent)));
  }
