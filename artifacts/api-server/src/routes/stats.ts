...
import { db, customersTable, ordersTable, campaignsTable, campaignStatsTable } from "@workspace/db";
...
router.get("/stats/overview", async (_req, res): Promise<void> => {
  ...
  const statsAgg = await db
    .select({
      totalSent: sql<number>`coalesce(sum(total_sent), 0)::int`,
      delivered: sql<number>`coalesce(sum(delivered), 0)::int`,
    })
  ...
  if (campaignIds.length > 0) {
    const stats = await db
      .select()
      .from(campaignStatsTable)
      .where(sql`campaign_id = ANY(${sql.raw(`ARRAY[${campaignIds.join(",")}]`)})`);
