...
import { useCreateCampaign, useSendCampaign, usePreviewSegment } from "../lib/api";
import type { AiChatMessage, AiReply } from "../lib/api-types";
...
export default function AiCampaigns() {
  ...
  const handleLaunch = () => {
    ...
    createMutation.mutate(
      { data: { name: `AI Campaign - ${new Date().toLocaleDateString()}`, segmentQuery: previewSegmentQuery, message: previewMessage, channel: previewChannel, sentByAi: true } },
      {
        onSuccess: (campaign) => {
          sendMutation.mutate(
            { id: campaign.id },
            {
              ...
              onError: () => toast({ title: "Launch Failed", description: "Failed to send the campaign.", variant: "destructive" }),
            }
          );
        },
        onError: () => toast({ title: "Creation Failed", description: "Failed to create the campaign.", variant: "destructive" }),
      }
    );
  };
          ...
          <div className="flex flex-col gap-4 pb-4">
            {messages.map((m, i) => (
              <div key={i} className={`flex gap-3 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
                ...
                <div className={`px-4 py-3 rounded-lg max-w-[80%] text-sm ${m.role === "user" ? "bg-primary text-primary-foreground rounded-tr-none" : "bg-secondary/50 text-foreground border border-border/50 rounded-tl-none leading-relaxed"}`}>
                  {m.content}
                </div>
              </div>
            ))}
        ...
        {previewSegmentQuery ? (
          <div className="flex-1 flex flex-col p-6 gap-6 overflow-y-auto">
            <div className="space-y-2">
              ...
              <div className="p-3 bg-secondary/30 rounded border border-border/50 font-mono text-xs text-primary/90 break-words">{previewSegmentQuery}</div>
            ...
            <div className="space-y-2">
              ...
              <div className="p-4 bg-secondary/30 rounded border border-border/50 text-sm whitespace-pre-wrap">{previewMessage}</div>
            </div>
